...
class PendingTaskListView extends StatefulWidget {
  PendingTaskListView({super.key,});

  @override
  State<PendingTaskListView> createState() => _PendingTaskListViewState();
}

class _PendingTaskListViewState extends State<PendingTaskListView> {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                ListView.builder(
                  itemCount: provider.pendingTodoList.length,
                  shrinkWrap: true,
                  physics: const NeverScrollableScrollPhysics(),
                  itemBuilder: (context, index) {
                    return ToDoBoxViewWidget(
                      description: provider.pendingTodoList[index].todo,
                      status: provider.pendingTodoList[index].completed==1?true:false,
                      taskId: provider.pendingTodoList[index].id,
                      onDelete: () async {
                        var result =await provider.deleteTask(provider.pendingTodoList[index].id);
                        if(result!){
